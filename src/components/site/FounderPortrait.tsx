...
export function FounderPortrait() {
  ...
  const onPick = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const { error: dbError } = await supabase
      .from("team_members")
      .upsert(
        { role_title: "Founder", name: FOUNDER_NAME, image_url: `${TEAM_BUCKET}:${objectPath}` },
    ...
    toast.success("Founder picture updated on the About page");
    await load();
    ...
    <div className="relative">
      <div className="aspect-square rounded-[2rem] bg-card border border-border shadow-pop p-1 rotate-3 hover:rotate-0 transition-transform duration-500 ring-1 ring-amber-500/20">
        <div className="h-full w-full rounded-[1.7rem] bg-primary flex items-center justify-center text-primary-foreground overflow-hidden">
          {url ? (
            <img src={url} alt={`${FOUNDER_NAME}, Founder & CEO of HumanLink`} className="h-full w-full object-cover" />
          ) : (
            <div className="text-center">
              ...
              <div className="mt-4 text-xl font-semibold">{FOUNDER_NAME}</div>
              <div className="text-sm opacity-90">Founder &amp; CEO, HumanLink</div>
