...
const PLAN_NAMES: Record<TierKey, string> = {
  pro: "HumanLink Pro",
  business: "HumanLink Business Promotion",
  ngo: "HumanLink NGO",
};
...
export function RazorpayCheckoutModal({ open, onOpenChange, defaultTier = "business", onSuccess }: { open: boolean; onOpenChange: (v: boolean) => void; defaultTier?: TierKey; onSuccess?: () => void }) {
  ...
  const [planIds, setPlanIds] = useState<Record<TierKey, string | null>>({ pro: null, business: null, ngo: null });
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const rows = data ?? [];
      setPlanIds({
        pro: rows.find((r) => r?.name === PLAN_NAMES.pro)?.id ?? null,
        business: rows.find((r) => r?.name === PLAN_NAMES.business)?.id ?? null,
        ngo: rows.find((r) => r?.name === PLAN_NAMES.ngo)?.id ?? null,
      });
    })();
  }, [open, defaultTier]);
  ...
  const subscribe = async () => {
    ...
    try {
      ...
      const msg = detail || "Checkout isn't available right now. Please try again.";
      toast.error(msg);
      setErrorMsg(msg);
      setStage("error");
    } finally {
          ...
          {stage === "pick" && <>
            <div className="grid gap-3 md:grid-cols-3">
              {CARDS.map((c) => {
                ...
                const Icon = c.key === "business" ? BriefcaseBusiness : c.key === "ngo" ? HeartHandshake : Crown;
                return <button key={c.key} type="button" onClick={() => setSelected(c.key)} className={cn("text-left rounded-2xl border p-4 transition-all", selectedCard ? "border-primary bg-primary/5 shadow-sm" : "border-border bg-card hover:border-primary/40")}>
                  <div className="flex items-center gap-2 font-semibold"><Icon className={cn("h-4 w-4", c.key === "business" ? "text-blue-600" : c.key === "ngo" ? "text-emerald-500" : "text-amber-500")} />{c.title}</div>
            ...
            <Button onClick={subscribe} disabled={loading} className="mt-6 w-full bg-primary text-primary-foreground hover:bg-primary/90 shadow-sm">
              {loading ? <><Loader2 className="h-4 w-4 mr-2 animate-spin" /> Loading secure checkout…</> : <>Subscribe Now · ₹{(active.price / 100).toLocaleString("en-IN")}</>}
            </Button>
